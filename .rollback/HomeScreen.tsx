...
import { groupByPlaylistCategories, deduplicateItems, extractBaseTitle } from '../utils/genreMap'
...
export default function HomeScreen() {
  ...
  const movieCategories = useMemo(() => {
    const rows = groupByPlaylistCategories(dedupedVods)
    return rows.map(([cat, items]) =>
      cat === 'Trending Now' ? [cat, trendingMovies] as [typeof cat, typeof items] : [cat, items] as [typeof cat, typeof items]
    )
  }, [dedupedVods, trendingMovies])

  const seriesCategories = useMemo(() => {
    const rows = groupByPlaylistCategories(dedupedSeries)
    return rows.map(([cat, items]) =>
      cat === 'Trending Now' ? [cat, trendingSeries] as [typeof cat, typeof items] : [cat, items] as [typeof cat, typeof items]
    )
